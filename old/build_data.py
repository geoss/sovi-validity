### single script to build the working data file ###
...
try:
    acs_sup1 = pd.read_csv(os.path.join(path,'sovi_acs_sup1.csv'), dtype=make_strings, skiprows=1)
...
w = ps.queen_from_shapefile(os.path.join(path,'USA_Counties_500k.shp'), idVariable='geoFIPS')
w.transform = 'R'
...
db1.QFAM = db1.QFAM.replace([np.inf,-np.inf], 0)
...
dbf = ps.open(os.path.join(path,'USA_Counties_500k.dbf'))
...
# Tests to validate equivalency (CAN SOMEONE EXPLAIN THIS TO ME, SETH)
