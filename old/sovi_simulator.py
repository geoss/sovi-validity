...
#        sims = 100  # number of simulations to run
        ...
        for name, sign, sample in input_names:
            ...
            columns = ['sim'+str(i) for i in range(sims)]
            sim_attribs = pd.DataFrame(index=data.Geo_FIPS, columns=columns)
            sim_sovis = pd.DataFrame(index=data.Geo_FIPS, columns=columns)
            ...
            for county in zinputs.index:
                ...
                #err = errors.ix[county][name]  # SE
                #err = errors.ix[county][name] * np.sqrt(sample_size)  # textbook convertion of SE to SD
                # SD from Brault (2014) that crosses from replicate weights SE to SD
                # sqrt( SE^2 * (R(1-eps)^2 / (R-1)) )
                #err = np.sqrt((errors.ix[county][name]**2) * ((sims*((1-0.5)**2))/(sims-1)))  
                ...
                if err > 0:   # need to catch the case of attributes with no uncertainty
                    ...
                    sim_attribs.ix[county] = np.random.normal(est, err, sims)
                else:
                    # return the value since no uncertainty
                    sim_attribs.ix[county] = est
            # prep to get data ready for PCA
            sim_attribs = sim_attribs.values.astype(float)
            sim_attribs = ZSCORE(sim_attribs)
            sim_attribs = pd.DataFrame(sim_attribs, index=data.Geo_FIPS, columns=columns)
            # compute SoVI for each simulation
            for sim in range(sims):
                zinputs[name] = sim_attribs['sim'+str(sim)]
                pca = SPSS_PCA(zinputs, reduce=True, varimax=True)
